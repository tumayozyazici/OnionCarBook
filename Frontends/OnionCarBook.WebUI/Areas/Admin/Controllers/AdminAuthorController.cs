...
namespace OnionCarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminAuthorController : Controller
    {
        ...
        public async Task<IActionResult> CreateAuthor(CreateAuthorDto createAuthorDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7126/api/Authors/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminAuthor", new { area = "Admin" });
            }
            return View();
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7126/api/Authors?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminAuthor", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateAuthor(UpdateAuthorDto updateAuthorDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7126/api/Authors/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminAuthor", new { area = "Admin" });
            }

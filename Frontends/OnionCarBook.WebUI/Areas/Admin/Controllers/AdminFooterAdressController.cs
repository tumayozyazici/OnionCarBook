...
namespace OnionCarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminFooterAdressController : Controller
    {
        ...
        public async Task<IActionResult> CreateFooterAdress(CreateFooterAdressDto createFooterAdressDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7126/api/FooterAdresses/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminFooterAdress", new { area = "Admin" });
            }
            return View();
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7126/api/FooterAdresses?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminFooterAdress", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateFooterAdress(UpdateFooterAdressDto updateFooterAdressDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7126/api/FooterAdresses/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminFooterAdress", new { area = "Admin" });
            }

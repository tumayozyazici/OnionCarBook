...
namespace OnionCarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminBannerController : Controller
    {
        ...
        public async Task<IActionResult> CreateBanner(CreateBannerDto createBannerDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7126/api/Banners/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminBanner", new { area = "Admin" });
            }
            return View();
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7126/api/Banners?id="+id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminBanner", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateBanner(UpdateBannerDto updateBannerDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7126/api/Banners/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminBanner", new { area = "Admin" });
            }

...
namespace OnionCarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminServiceController : Controller
    {
        ...
        public async Task<IActionResult> CreateService(CreateServiceDto createServiceDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7126/api/Services/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7126/api/Services?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminService", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceDto updateServiceDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7126/api/Services/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Index), "AdminService", new { area = "Admin" });
            }
